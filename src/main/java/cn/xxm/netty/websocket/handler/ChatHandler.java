...
public class ChatHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
        ...
        //    channel.writeAndFlush(new TextWebSocketFrame("[服务器接在]"+LocalDateTime.now() +"接收到消息,消息为"+content));
        //}

        // 下面这个方法,和上面的for循环,一致
        clients.writeAndFlush(new TextWebSocketFrame("[服务器接在]"+LocalDateTime.now() +"接收到消息,消息为"+content));
